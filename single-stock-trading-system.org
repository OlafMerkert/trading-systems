...
(eazy-gnuplot:with-plots (*standard-output* :debug nil)
  (eazy-gnuplot:gp-setup :terminal '(pngcairo) :output "plot-1.png" :bars 2)
  (eazy-gnuplot:plot (lambda ()
                       (map nil (lambda (d o l h c)
                                  ;; date open low high close
                                  (format t "~&~A ~A ~A ~A ~A" d o l h c))
                            (stock-date *current-stock*)
                            (stock-open *current-stock*)
                            (stock-low *current-stock*)
                            (stock-high *current-stock*)
                            (stock-close *current-stock*)))
                     :with 'financebars))
#+end_src

file:plot-1.png
*** TODO fix date format for gnuplot
