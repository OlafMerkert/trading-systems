...
Now that everything is in place, let's try the trader.
#+name: rand-av-cmp-trader-test
#+begin_src lisp
(defpar *current-trader*
        (make-instance 'randomised-average-cmp-trader
                       :observation-frame-length 10
                       :trader-stock *current-stock*))

(simulate-trader *current-trader* 100000 :start-after 20)
#+end_src
...
#+end_src
*** DONE investigate why we make so much money
